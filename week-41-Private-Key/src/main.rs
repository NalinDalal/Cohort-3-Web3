...
fn main() -> Result<(), Error> {
    ...
    match opts {
        ...
        Options::AggSendStepOne { keypair } => {
            ...
            println!(
                "Message 1: {} (send to all other parties)",
                first_msg.serialize_bs58()
            );
            println!(
        ...
        Options::AggregateSignaturesAndBroadcast {
            signatures,
            amount,
            to,
            memo,
            ...
            let tx =
                tss::sign_and_broadcast(amount, to, memo, recent_block_hash, keys, signatures)?;
            let rpc_client = RpcClient::new(net.get_cluster_url().to_string());
            let sig = rpc_client
                .send_transaction(&tx)
                .map_err(Error::SendTransactionFailed)?;
